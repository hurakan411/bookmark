...
class OptimalFolderStructureRequest(BaseModel):
    ...
    max_depth: int = 3  # フォルダの最大階層数（デフォルト3）
...
async def analyze_folder_structure(request: OptimalFolderStructureRequest):
    ...
    try:
        ...
        # プロンプトの作成
        max_depth = min(request.max_depth, 10)  # 最大10層まで
...
- **フォルダ**: 大分類・カテゴリ（例: 仕事、趣味、プロジェクト名、テーマ別）
  ...
  - **階層は最大{max_depth}層まで**（深すぎる階層は避ける）
- **タグ**: コンテンツの特徴・属性を表すキーワード
  - 横断的な分類（複数のフォルダにまたがる特徴）
  - 検索・フィルタリング用
...
1. **新規・階層構造の提案（必須）**
   ...
   - 親子関係を活用した階層構造（最大{max_depth}層まで）
...
4. 既存のフォルダリストに適切なものがない場合は「未分類」を選んでください
...
- 日本語で回答してください"""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたはブックマーク整理の専門家です。各ブックマークの内容を分析し、最も適切なフォルダに分類してください。階層の深いフォルダ（第2階層、第3階層）を積極的に使用して、より詳細で整理された分類を行ってください。必ずJSON形式で回答してください。"
                },
                {
                    "role": "user",
                    "content": prompt
